...
export function repositorySourcePathSafe(path: string): boolean {
  return path.length >= 1 && path.length <= 250 && path.split('/').every(part =>
    ...
    && !/^(?:\.env.*|\.netrc|\.npmrc|\.pypirc|\.git|\.gitnexus|\.context|\.dharma|\.dharma-managed|\.dharma-activation-.*|\.codex-pr-worktrees|\.worktrees|node_modules|dist|build|\.ssh|\.aws|\.gnupg|id_rsa|id_ed25519)$/i.test(part)
