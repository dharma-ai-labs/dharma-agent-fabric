...
test('bootstrap rejects a connected relay from an older CLI release', async () => {
  await assert.rejects(
    () => waitForRelayReadiness({
      ...
      expectedVersion: '0.2.99',
      attempts: 1,
    }),
    /relay_version_mismatch:0\.2\.80:0\.2\.99/,
  );
  await assert.rejects(
    () => waitForRelayReadiness({
      processState: async () => 'running',
      probe: async () => ({ ok: true, connected: false, organizationId: 'org_test',
        deviceId: 'device_test', relayVersion: '0.2.99' }),
      expectedVersion: '0.2.99',
      attempts: 1,
    }),
    /relay_session_unacknowledged/,
...
test('relay probe opens an authenticated session without polling or leasing work', async () => {
  ...
  const result = await probeRelayConnection(async () => ({
    ...
    openSession: async (version?: string) => {
      sessions += 1;
      assert.equal(version, '0.2.106');
      return { ok: true };
    },
  }));
  assert.equal(sessions, 1);
  assert.deepEqual(result, {
    ok: true, connected: true, organizationId: 'org_test', deviceId: 'device_test', relayVersion: '0.2.106',
  });
});
