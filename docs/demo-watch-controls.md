...
The shared supervisor polls the local registry every two seconds while keeping each scope's configured cycle interval (60 seconds by default). It uses a monotonic scheduling clock, four concurrent cycles by default, and a 30-second cycle deadline. A timed-out operation retains its slot until it actually settles, so a non-cooperative provider cannot cause duplicate or unbounded work. The bounded 32-scope scheduling test fits the five-minute observation target for responsive cycles; it does not guarantee provider availability, publication completion, or safe native activation within that window.
