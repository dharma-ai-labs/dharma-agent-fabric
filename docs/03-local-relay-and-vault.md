...
Completed onboarding starts a detached local supervisor. It restarts the relay after an unexpected process exit with bounded backoff. The complete bootstrap also registers a per-user Windows logon task or Linux systemd user service, using the pinned repository launcher and canonical policy path without storing a grant or device credential in the startup entry. `dharma status` reports the relay, supervisor, startup registration, and last successful poll. After reboot, the enrolled operating-system user must sign in; the relay then resumes without another prompt. `dharma relay stop` stops the current process; `dharma relay autostart disable` removes the future logon entry. Linux without a systemd user bus, WSL without user-session startup, and macOS are not yet reboot-autonomous and must report that limitation rather than a complete startup receipt.
